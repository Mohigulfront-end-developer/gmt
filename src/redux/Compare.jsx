...
const compareProductsSlice = createSlice({
  ...
  reducers: {
    ...
    handleCompare : (state, action) => {
      ...
      const compareProduct = state.compare.findIndex(product => product.id === item.id);
      if(compareProduct === -1){
        state.compare = [ ...state.compare, item ]
      } else {
        state.compare = state.compare.filter((product) => product.id !== item.id);
      }
...
export const { handleCompare, addCompareProduct, removeCompareProduct, removeCompare, addAllCompareProducts } =
